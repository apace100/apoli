...
public class FleeMobBehavior extends MobBehavior {
    ...
    public FleeMobBehavior(int priority, float fleeDistance, double slowSpeed, double fastSpeed) {
        ...
        taskMap.put(ApoliActivities.AVOID, new Pair(ImmutableList.of(GoToRememberedPositionTask.toEntity(ApoliMemoryModuleTypes.AVOID_TARGET, (float) this.speed, Math.round(this.fleeDistance), false), new ForgetTask<>((mob) -> FleeMobBehavior.shouldForgetAvoidTask(mob, this), ApoliMemoryModuleTypes.AVOID_TARGET)), ImmutableList.of(new com.mojang.datafixers.util.Pair<>(ApoliMemoryModuleTypes.AVOID_TARGET, MemoryModuleState.VALUE_PRESENT))));
        biEntityPredicate = (pair) -> pair.getLeft().getBoundingBox().intersects(pair.getRight().getBoundingBox().expand(fleeDistance));
    }
    ...
    protected void updateMemories(MobEntity mob, LivingEntity powerHolder) {
        if (activeEntities.contains(mob) && mob.getBrain().hasMemoryModule(ApoliMemoryModuleTypes.AVOID_TARGET) && mob.getBrain().getOptionalMemory(ApoliMemoryModuleTypes.AVOID_TARGET).isPresent() && mob.getBrain().getOptionalMemory(ApoliMemoryModuleTypes.AVOID_TARGET).get() != powerHolder) {
        ...
        mob.getBrain().remember(ApoliMemoryModuleTypes.AVOID_TARGET, powerHolder, 100L);
