...
public class ModifyPlayerSpawnPowerType extends PowerType {
    ...
    public ModifyPlayerSpawnPowerType(Power power, LivingEntity entity, RegistryKey<World> dimension, float dimensionDistanceMultiplier, RegistryKey<Biome> biome, SpawnStrategy spawnStrategy, RegistryKey<Structure> structure, SoundEvent spawnSound) {
        super(power, entity);
    ...
    public void teleportToModifiedSpawn() {
        ...
        if (tpPos == null) {
            ...
            Apoli.LOGGER.warn("Power {} could not find a suitable spawnpoint for {}! Teleporting to the desired location directly...", this.getPower().getId(), entity.getNameForScoreboard());
        }
    ...
    public Pair<ServerWorld, BlockPos> getSpawn(boolean isSpawnObstructed) {
        ...
        if (targetDimension == null) {
            Apoli.LOGGER.warn("Power {} could not set {}'s spawnpoint at dimension \"{}\" as it's not registered! Falling back to default spawnpoint...", this.getPower().getId(), entity.getNameForScoreboard(), dimension.getValue());
            return null;
        }
    ...
    private Optional<BlockPos> getBiomePos(ServerWorld targetDimension, BlockPos originPos) {
        ...
        Optional<Biome> targetBiome = targetDimension.getRegistryManager().get(RegistryKeys.BIOME).getOrEmpty(biome);
        if (targetBiome.isEmpty()) {
            Apoli.LOGGER.warn("Power {} could not set {}'s spawnpoint at biome \"{}\" as it's not registered in dimension \"{}\".", this.getPower().getId(), entity.getNameForScoreboard(), biome, dimension.getValue());
        ...
        if (targetBiomePos != null) {
            ...
            Apoli.LOGGER.warn("Power {} could not set {}'s spawnpoint at biome \"{}\" as it couldn't be found in dimension \"{}\".", this.getPower().getId(), entity.getNameForScoreboard(), biome, dimension.getValue());
            return Optional.empty();
        }
    ...
    private Optional<Pair<BlockPos, Structure>> getStructurePos(World world, RegistryKey<Structure> structure, TagKey<Structure> structureTag, RegistryKey<World> dimension) {
        ...
        if (structurePos == null) {
            Apoli.LOGGER.warn("Power {} could not set {}'s spawnpoint at structure \"{}\" as it couldn't be found in dimension \"{}\".", this.getPower().getId(), entity.getNameForScoreboard(), structureTagOrName, dimension.getValue());
            return Optional.empty();
        }
