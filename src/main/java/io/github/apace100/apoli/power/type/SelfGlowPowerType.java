...
public class SelfGlowPowerType extends PowerType {
    ...
    public SelfGlowPowerType(Power power, LivingEntity entity, Predicate<Entity> entityCondition, Predicate<Pair<Entity, Entity>> biEntityCondition, boolean useTeams, float red, float green, float blue) {
        super(power, entity);
    ...
    public boolean doesApply(Entity viewer) {
        return (entityCondition == null || entityCondition.test(viewer))
            && (biEntityCondition == null || biEntityCondition.test(new Pair<>(viewer, entity)));
    }
